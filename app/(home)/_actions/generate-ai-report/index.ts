...
import { generateAiReportChema, GenerateAiReportSchema } from "./schema";
...
export const generateAiReport = async ({
  ...
  generateAiReportChema.parse({ month, year });
  ...
  const transactions = await db.transaction.findMany({
    where: {
      date: {
        gte: new Date(
          `${year ?? new Date().getFullYear()}-${month ?? new Date().getMonth() + 1}-01`,
        ),
        lt: new Date(
          `${year ?? new Date().getFullYear()}-${month ?? new Date().getMonth() + 1}-31`,
        ),
      },
    },
  });
